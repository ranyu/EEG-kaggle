...
from lasagne.layers import InputLayer, GRULayer,DropoutLayer, FeaturePoolLayer,DenseLayer,Conv2DLayer,Conv1DLayer,MaxPool1DLayer,RecurrentLayer,LSTMLayer
from lasagne.updates import nesterov_momentum,adagrad,rmsprop,adadelta
...
def create_net(train_source, test_source, batch_size=128, max_epochs=20): 
    ...
    layers = [
        ...
        #LF(Conv2DLayer, num_filters=4, filter_size=1, nonlinearity=None),
        ...
        #LF(DropoutLayer, p=0.5),
        #LF(DenseLayer, num_units=dense),
        LF(LSTMLayer, num_units=dense),
        LF(DropoutLayer, p=0.5),
        LF(LSTMLayer, num_units=dense),
        LF(DropoutLayer, p=0.5),
        #LF(DenseLayer, num_units=dense),
        #LF(FeaturePoolLayer, pool_size=2),
        #LF(DenseLayer, num_units=256),
        #LF(FeaturePoolLayer, pool_size=2),
        #LF(RecurrentLayer, num_units=dense),
        #LF(DropoutLayer, p=0.5),
...
def train(factory, subj, max_epochs=20, valid_series=[3,6], params=None):
...
def train_all(factory, max_epochs=20, init_epochs=15, valid_series=[3,6]):
    info = {}
    params = None
...
if __name__ == "__main__":
    train_info = train_all(create_net, max_epochs=10) # Training for longer would likley be better
    make_submission(train_info, "grasp_epoch_new.csv")
